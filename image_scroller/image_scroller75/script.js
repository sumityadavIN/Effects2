...
// Manual word splitting (replaces SplitType)
function setAndAnimateTitle(title) {
  const words = title.split(" ");
  projectTitleElement.innerHTML = words.map(word =>
    `<span class="word" style="display:inline-block; overflow:hidden;"><span style="display:inline-block; transform:translateY(100%);">${word}</span></span>`
  ).join(" ");
  titleWords = projectTitleElement.querySelectorAll(".word > span");
}
...
function updateVisibleItems() {
  ...
  for (let row = startRow; row <= endRow; row++) {
    for (let col = startCol; col <= endCol; col++) {
      ...
      img.src = `./public/img${itemNum}.jpg`;
